"""
Modulo Motore di Visualizzazione - VERSIONE CORRETTA
...
class VisualizationEngine:
    ...
    def create_visualizations(self, data: pd.DataFrame, 
        ...
        try:
            ...
            logger.error(f"Errore creando grafici distribuzioni: {str(e)}")
        ...
        try:
            ...
            logger.error(f"Errore creando heatmap correlazioni: {str(e)}")
        ...
        try:
            ...
            logger.error(f"Errore creando visualizzazioni PCA: {str(e)}")
        ...
        try:
            ...
            logger.error(f"Errore creando grafici serie temporali: {str(e)}")
        ...
        try:
            ...
            logger.error(f"Errore creando grafici clustering: {str(e)}")
        
        return visualizations
    
    def _create_distribution_plots(self, data: pd.DataFrame) -> List:
        """Crea grafici distribuzione per colonne numeriche"""
        charts = []
        numeric_cols = data.select_dtypes(include=[np.number]).columns.tolist()
        
        # Crea subplot per prime 6 colonne numeriche
    ...
    def _create_pca_visualizations(self, pca_results: Dict) -> List:
        ...
        if 'components' in pca_results and 'explained_variance_ratio' in pca_results:
            ...
            # Grafico loadings componenti (se dati trasformati disponibili)
    ...
    def _create_time_series_plots(self, data: pd.DataFrame, ts_results: Dict) -> List:
        ...
        for col, results in list(ts_results.items())[:3]:  # Limita a 3 grafici
            if col in data.columns:
                try:
                    fig = go.Figure()
                    
                    # FIX: Converti range in lista per compatibilità Plotly
                    ...
                    # Aggiungi linea di trend se disponibile
                    ...
                    logger.error(f"Errore creando grafico serie temporali per {col}: {str(e)}")
                    continue
    ...
    def _create_clustering_plots(self, clustering_results: Dict) -> List:
        ...
        if 'kmeans' in clustering_results:
            ...
            if 'inertias' in kmeans_data and 'silhouette_scores' in kmeans_data:
                fig = make_subplots(
                    rows=1, cols=2,
                    subplot_titles=['Metodo Elbow', 'Punteggio Silhouette']
                )
    ...
    def create_custom_chart(self, chart_type: str, data: pd.DataFrame, 
        ...
        try:
            ...
            logger.error(f"Errore creando grafico personalizzato: {str(e)}")
            return None
    
    def create_ai_insights_visualization(self, ai_results: Dict) -> List:
        """Crea visualizzazioni per insight IA"""
        charts = []
        
        # Crea grafici per risultati di diversi agenti IA
            ...
            if isinstance(results, dict) and 'visualizations' in results:
                for viz in results['visualizations']:
                    try:
                        ...
                        logger.error(f"Errore creando visualizzazione IA: {str(e)}")
        
        return charts
    
    def _create_chart_from_spec(self, spec: Dict) -> go.Figure:
        """Crea grafico da specifica generata dall'IA"""
        chart_type = spec.get('type')
        data = spec.get('data')
        title = spec.get('title', 'Grafico Generato dall\'IA')
